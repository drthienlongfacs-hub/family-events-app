...
function extractValidDate(text, pubDateFallback) {
    ...
    if (match) {
        ...
        extracted = new Date(d.getTime() + 86400000);
    }

    // Strict temporal bounding (RCA-042): 
    // If the event date is less than (Today - 1 day), it's expired.
    ...
    const tasks = ACTIVE_PROVIDERS.map(async (provider) => {
        try {
            ...
            for (let i = 0; i < itemElements.length; i++) {
                ...
                // RCA-042: 100% URL Verification (Block 404 Fake/Dead Links)
                let isAlive = false;
                try {
                    // Try a fast HEAD request over Axios to verify the upstream link exists
                    await axios.head(link, { timeout: 3000 });
                    isAlive = true;
                } catch (err) {
                    // If it specifically returns 404, the link is truly dead/fake.
                    // Many sites block HEAD requests (403/405/500/Timeout), so we'll only kill strict 404s to avoid false positives.
                    if (err.response && err.response.status === 404) isAlive = false;
                    else isAlive = true; // Assume alive if blocked by WAF or timeout
                }
                ...
                const validIsoDate = extractValidDate(combinedTextForAnalysis, pubDate);
                if (!validIsoDate) {
