        ...
        const tasks = ACTIVE_PROVIDERS.map(async (provider) => {
            try {
                ...
                for (let i = 0; i < itemElements.length; i++) {
                    ...
                    // RELAXATION: If event is SAFE and achieves at least 40% threshold (e.g. Interactive + Implicit Location).
                    // This prevents over-filtering of our highly constrained Google News domains.
                    if (evalResult.isSafe && evalResult.score >= 40) {
                        items.push({
